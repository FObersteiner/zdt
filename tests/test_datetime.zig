...
test "Datetime Unix epoch roundtrip" {
    ...
    var unix_from_int = try Datetime.fromUnix(42, Duration.Resolution.second, null, null);
    ...
    unix_from_int = try Datetime.fromUnix(42001, Duration.Resolution.millisecond, null, null);
    try testing.expect(std.meta.eql(unix_from_fields, unix_from_int));
    ...
    unix_from_int = try Datetime.fromUnix(42001001, Duration.Resolution.microsecond, null, null);
    try testing.expect(std.meta.eql(unix_from_fields, unix_from_int));
    ...
    unix_from_int = try Datetime.fromUnix(42001001001, Duration.Resolution.nanosecond, null, null);
    try testing.expect(std.meta.eql(unix_from_fields, unix_from_int));
...
test "Fields can represent leap second, Unix cannot" {
    ...
    const dt_from_int = try Datetime.fromUnix(60, Duration.Resolution.second, null, null);
...
test "Datetime Min Max fields vs seconds roundtrip" {
    const max_from_seconds = try Datetime.fromUnix(Datetime.unix_s_max, Duration.Resolution.second, null, null);
    ...
    const min_from_seconds = try Datetime.fromUnix(Datetime.unix_s_min, Duration.Resolution.second, null, null);
    try testing.expect(std.meta.eql(min_from_fields, min_from_seconds));

    const too_large_s = Datetime.fromUnix(Datetime.unix_s_max + 1, Duration.Resolution.second, null, null);
    try testing.expectError(ZdtError.UnixOutOfRange, too_large_s);
    const too_large_ns = Datetime.fromUnix(@as(i128, Datetime.unix_s_max + 1) * std.time.ns_per_s, Duration.Resolution.second, null, null);
    try testing.expectError(ZdtError.UnixOutOfRange, too_large_ns);
    const too_small_s = Datetime.fromUnix(Datetime.unix_s_min - 1, Duration.Resolution.second, null, null);
    try testing.expectError(ZdtError.UnixOutOfRange, too_small_s);
    const too_small_ns = Datetime.fromUnix(@as(i128, Datetime.unix_s_min - 1) * std.time.ns_per_s, Duration.Resolution.second, null, null);
...
test "format offset" {
    ...
    var dt = try Datetime.fromFields(.{ .year = 2021, .month = 2, .day = 18, .hour = 17, .utc_offset = offset });
    ...
    dt = try Datetime.fromFields(.{ .year = 2021, .month = 2, .day = 18, .hour = 17, .utc_offset = offset });
    str.clearAndFree();
...
test "compare Unix time" {
    ...
    var a = try Datetime.fromFields(.{ .year = 2021, .month = 2, .day = 18, .hour = 17, .utc_offset = offset });
...
test "compare wall time" {
    ...
    const b = try Datetime.fromFields(.{ .year = 2021, .month = 2, .day = 18, .hour = 18, .nanosecond = 42, .utc_offset = offset });
...
test "unix nanoseconds, fields" {
  ...
  // 2006-02-13T07:49:30.157447+00:00 :
    dt_from_unix = try Datetime.fromUnix(1139816970157447204, Duration.Resolution.nanosecond, null, null);
  ...
  // 1911-07-04T16:58:55.335801+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1845961264664198296, Duration.Resolution.nanosecond, null, null);
  ...
  // 1976-04-03T16:17:57.112616+00:00 :
    dt_from_unix = try Datetime.fromUnix(197396277112616585, Duration.Resolution.nanosecond, null, null);
  ...
  // 1993-06-15T03:31:07.631347+00:00 :
    dt_from_unix = try Datetime.fromUnix(740115067631347709, Duration.Resolution.nanosecond, null, null);
  ...
  // 1965-09-10T21:10:04.900889+00:00 :
    dt_from_unix = try Datetime.fromUnix(-135917395099110276, Duration.Resolution.nanosecond, null, null);
  ...
  // 2025-05-11T07:55:43.346194+00:00 :
    dt_from_unix = try Datetime.fromUnix(1746950143346194291, Duration.Resolution.nanosecond, null, null);
  ...
  // 2033-09-19T08:52:23.419743+00:00 :
    dt_from_unix = try Datetime.fromUnix(2010732743419743984, Duration.Resolution.nanosecond, null, null);
  ...
  // 2011-11-25T20:30:45.142200+00:00 :
    dt_from_unix = try Datetime.fromUnix(1322253045142200462, Duration.Resolution.nanosecond, null, null);
  ...
  // 1974-07-17T16:08:44.216813+00:00 :
    dt_from_unix = try Datetime.fromUnix(143309324216813127, Duration.Resolution.nanosecond, null, null);
  ...
  // 1970-06-15T23:04:13.701167+00:00 :
    dt_from_unix = try Datetime.fromUnix(14339053701167226, Duration.Resolution.nanosecond, null, null);
  ...
  // 1959-03-24T23:34:55.696166+00:00 :
    dt_from_unix = try Datetime.fromUnix(-339985504303833582, Duration.Resolution.nanosecond, null, null);
  ...
  // 2005-03-22T04:41:50.599516+00:00 :
    dt_from_unix = try Datetime.fromUnix(1111466510599516821, Duration.Resolution.nanosecond, null, null);
  ...
  // 2057-10-16T13:20:17.421883+00:00 :
    dt_from_unix = try Datetime.fromUnix(2770464017421883411, Duration.Resolution.nanosecond, null, null);
  ...
  // 1924-12-02T17:41:26.878193+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1422598713121806117, Duration.Resolution.nanosecond, null, null);
  ...
  // 1968-04-22T02:14:14.542383+00:00 :
    dt_from_unix = try Datetime.fromUnix(-53473545457616547, Duration.Resolution.nanosecond, null, null);
  ...
  // 2071-12-05T04:41:36.510627+00:00 :
    dt_from_unix = try Datetime.fromUnix(3216516096510627205, Duration.Resolution.nanosecond, null, null);
  ...
  // 2050-04-16T18:20:18.629659+00:00 :
    dt_from_unix = try Datetime.fromUnix(2533746018629659335, Duration.Resolution.nanosecond, null, null);
  ...
  // 2040-11-07T18:01:31.246681+00:00 :
    dt_from_unix = try Datetime.fromUnix(2235924091246681545, Duration.Resolution.nanosecond, null, null);
  ...
  // 2095-10-06T02:49:51.563857+00:00 :
    dt_from_unix = try Datetime.fromUnix(3968707791563857370, Duration.Resolution.nanosecond, null, null);
  ...
  // 1906-02-17T08:23:28.253135+00:00 :
    dt_from_unix = try Datetime.fromUnix(-2015595391746864347, Duration.Resolution.nanosecond, null, null);
  ...
  // 2030-04-26T08:44:23.368914+00:00 :
    dt_from_unix = try Datetime.fromUnix(1903423463368914145, Duration.Resolution.nanosecond, null, null);
  ...
  // 2093-01-17T14:57:50.411423+00:00 :
    dt_from_unix = try Datetime.fromUnix(3883042670411423755, Duration.Resolution.nanosecond, null, null);
  ...
  // 2087-08-25T06:40:03.604387+00:00 :
    dt_from_unix = try Datetime.fromUnix(3712632003604387205, Duration.Resolution.nanosecond, null, null);
  ...
  // 1996-03-05T04:01:33.200875+00:00 :
    dt_from_unix = try Datetime.fromUnix(825998493200875839, Duration.Resolution.nanosecond, null, null);
  ...
  // 2014-07-30T05:41:43.718062+00:00 :
    dt_from_unix = try Datetime.fromUnix(1406698903718062404, Duration.Resolution.nanosecond, null, null);
  ...
  // 2079-01-02T08:34:17.727757+00:00 :
    dt_from_unix = try Datetime.fromUnix(3439874057727757792, Duration.Resolution.nanosecond, null, null);
  ...
  // 2089-04-15T16:45:14.981203+00:00 :
    dt_from_unix = try Datetime.fromUnix(3764421914981203364, Duration.Resolution.nanosecond, null, null);
  ...
  // 2001-07-15T23:49:30.572376+00:00 :
    dt_from_unix = try Datetime.fromUnix(995240970572376801, Duration.Resolution.nanosecond, null, null);
  ...
  // 2085-12-16T15:41:54.673785+00:00 :
    dt_from_unix = try Datetime.fromUnix(3659355714673785261, Duration.Resolution.nanosecond, null, null);
  ...
  // 2068-01-31T16:03:15.928089+00:00 :
    dt_from_unix = try Datetime.fromUnix(3095251395928089236, Duration.Resolution.nanosecond, null, null);
  ...
  // 2053-07-16T04:39:03.670448+00:00 :
    dt_from_unix = try Datetime.fromUnix(2636253543670448791, Duration.Resolution.nanosecond, null, null);
  ...
  // 2049-01-26T19:12:56.022732+00:00 :
    dt_from_unix = try Datetime.fromUnix(2495301176022732233, Duration.Resolution.nanosecond, null, null);
  ...
  // 2057-07-27T21:06:01.959375+00:00 :
    dt_from_unix = try Datetime.fromUnix(2763493561959375132, Duration.Resolution.nanosecond, null, null);
  ...
  // 2014-02-23T21:56:38.419373+00:00 :
    dt_from_unix = try Datetime.fromUnix(1393192598419373852, Duration.Resolution.nanosecond, null, null);
  ...
  // 1970-05-05T22:45:53.859058+00:00 :
    dt_from_unix = try Datetime.fromUnix(10795553859058913, Duration.Resolution.nanosecond, null, null);
  ...
  // 1976-01-28T07:20:41.147095+00:00 :
    dt_from_unix = try Datetime.fromUnix(191661641147095672, Duration.Resolution.nanosecond, null, null);
  ...
  // 1971-05-08T08:19:43.172751+00:00 :
    dt_from_unix = try Datetime.fromUnix(42538783172751947, Duration.Resolution.nanosecond, null, null);
  ...
  // 2057-04-14T06:25:36.565828+00:00 :
    dt_from_unix = try Datetime.fromUnix(2754455136565828242, Duration.Resolution.nanosecond, null, null);
  ...
  // 1951-06-13T01:15:34.804713+00:00 :
    dt_from_unix = try Datetime.fromUnix(-585528265195286599, Duration.Resolution.nanosecond, null, null);
  ...
  // 1921-09-22T21:06:50.877267+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1523415189122732770, Duration.Resolution.nanosecond, null, null);
  ...
  // 1975-06-19T10:02:26.769740+00:00 :
    dt_from_unix = try Datetime.fromUnix(172404146769740667, Duration.Resolution.nanosecond, null, null);
  ...
  // 2062-03-30T22:59:52.749455+00:00 :
    dt_from_unix = try Datetime.fromUnix(2910985192749455412, Duration.Resolution.nanosecond, null, null);
  ...
  // 1929-11-13T00:23:14.749022+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1266536205250977058, Duration.Resolution.nanosecond, null, null);
  ...
  // 2034-05-16T00:49:01.138436+00:00 :
    dt_from_unix = try Datetime.fromUnix(2031353341138436500, Duration.Resolution.nanosecond, null, null);
  ...
  // 2016-08-15T19:58:24.806277+00:00 :
    dt_from_unix = try Datetime.fromUnix(1471291104806277326, Duration.Resolution.nanosecond, null, null);
  ...
  // 2027-09-19T10:04:59.644137+00:00 :
    dt_from_unix = try Datetime.fromUnix(1821348299644137670, Duration.Resolution.nanosecond, null, null);
  ...
  // 1903-08-16T20:57:39.201585+00:00 :
    dt_from_unix = try Datetime.fromUnix(-2094692540798414766, Duration.Resolution.nanosecond, null, null);
  ...
  // 1980-10-20T09:43:08.732900+00:00 :
    dt_from_unix = try Datetime.fromUnix(340882988732900487, Duration.Resolution.nanosecond, null, null);
  ...
  // 2077-12-18T13:15:08.880882+00:00 :
    dt_from_unix = try Datetime.fromUnix(3407058908880882376, Duration.Resolution.nanosecond, null, null);
  ...
  // 2020-03-29T06:39:33.998392+00:00 :
    dt_from_unix = try Datetime.fromUnix(1585463973998392555, Duration.Resolution.nanosecond, null, null);
  ...
  // 1977-12-18T19:07:32.591266+00:00 :
    dt_from_unix = try Datetime.fromUnix(251320052591266848, Duration.Resolution.nanosecond, null, null);
  ...
  // 1905-08-26T12:42:48.251099+00:00 :
    dt_from_unix = try Datetime.fromUnix(-2030699831748900962, Duration.Resolution.nanosecond, null, null);
  ...
  // 1911-10-29T00:43:19.724354+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1835911000275645366, Duration.Resolution.nanosecond, null, null);
  ...
  // 2021-01-29T11:08:32.946010+00:00 :
    dt_from_unix = try Datetime.fromUnix(1611918512946010890, Duration.Resolution.nanosecond, null, null);
  ...
  // 1995-01-26T00:52:22.256639+00:00 :
    dt_from_unix = try Datetime.fromUnix(791081542256639804, Duration.Resolution.nanosecond, null, null);
  ...
  // 2081-12-14T10:58:42.784782+00:00 :
    dt_from_unix = try Datetime.fromUnix(3532935522784782786, Duration.Resolution.nanosecond, null, null);
  ...
  // 2058-06-16T04:11:26.495154+00:00 :
    dt_from_unix = try Datetime.fromUnix(2791426286495154255, Duration.Resolution.nanosecond, null, null);
  ...
  // 2041-08-25T01:38:07.571380+00:00 :
    dt_from_unix = try Datetime.fromUnix(2261007487571380607, Duration.Resolution.nanosecond, null, null);
  ...
  // 1932-07-27T20:53:06.297782+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1181185613702217311, Duration.Resolution.nanosecond, null, null);
  ...
  // 2031-06-08T17:03:58.474063+00:00 :
    dt_from_unix = try Datetime.fromUnix(1938704638474063316, Duration.Resolution.nanosecond, null, null);
  ...
  // 1986-08-31T16:31:49.994491+00:00 :
    dt_from_unix = try Datetime.fromUnix(525889909994491835, Duration.Resolution.nanosecond, null, null);
  ...
  // 2086-10-28T21:01:36.630063+00:00 :
    dt_from_unix = try Datetime.fromUnix(3686677296630063857, Duration.Resolution.nanosecond, null, null);
  ...
  // 2046-11-07T03:20:51.843511+00:00 :
    dt_from_unix = try Datetime.fromUnix(2425173651843511531, Duration.Resolution.nanosecond, null, null);
  ...
  // 2035-09-12T05:07:54.992775+00:00 :
    dt_from_unix = try Datetime.fromUnix(2073186474992775101, Duration.Resolution.nanosecond, null, null);
  ...
  // 2020-10-27T07:14:33.322766+00:00 :
    dt_from_unix = try Datetime.fromUnix(1603782873322766604, Duration.Resolution.nanosecond, null, null);
  ...
  // 2022-04-25T22:00:05.144375+00:00 :
    dt_from_unix = try Datetime.fromUnix(1650924005144375799, Duration.Resolution.nanosecond, null, null);
  ...
  // 1919-03-24T22:16:36.922882+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1602294203077117410, Duration.Resolution.nanosecond, null, null);
  ...
  // 1934-08-30T22:17:10.073246+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1115170969926753570, Duration.Resolution.nanosecond, null, null);
  ...
  // 1911-04-06T14:31:43.402977+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1853659696597022699, Duration.Resolution.nanosecond, null, null);
  ...
  // 1918-04-05T04:26:09.604044+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1632857630395955914, Duration.Resolution.nanosecond, null, null);
  ...
  // 1927-12-28T07:55:17.978510+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1325779482021489161, Duration.Resolution.nanosecond, null, null);
  ...
  // 2078-01-15T20:58:43.722981+00:00 :
    dt_from_unix = try Datetime.fromUnix(3409505923722981966, Duration.Resolution.nanosecond, null, null);
  ...
  // 2093-08-14T20:32:19.511857+00:00 :
    dt_from_unix = try Datetime.fromUnix(3901120339511857368, Duration.Resolution.nanosecond, null, null);
  ...
  // 1920-02-27T11:16:24.349769+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1572957815650230821, Duration.Resolution.nanosecond, null, null);
  ...
  // 2032-05-15T22:32:49.528714+00:00 :
    dt_from_unix = try Datetime.fromUnix(1968273169528714623, Duration.Resolution.nanosecond, null, null);
  ...
  // 2020-06-27T17:00:26.715258+00:00 :
    dt_from_unix = try Datetime.fromUnix(1593277226715258764, Duration.Resolution.nanosecond, null, null);
  ...
  // 2091-12-21T08:27:54.060431+00:00 :
    dt_from_unix = try Datetime.fromUnix(3849064074060431958, Duration.Resolution.nanosecond, null, null);
  ...
  // 2087-12-04T04:15:22.007149+00:00 :
    dt_from_unix = try Datetime.fromUnix(3721349722007149573, Duration.Resolution.nanosecond, null, null);
  ...
  // 1947-12-22T15:17:09.971666+00:00 :
    dt_from_unix = try Datetime.fromUnix(-695119370028333617, Duration.Resolution.nanosecond, null, null);
  ...
  // 1978-03-21T18:31:40.719801+00:00 :
    dt_from_unix = try Datetime.fromUnix(259353100719801983, Duration.Resolution.nanosecond, null, null);
  ...
  // 1996-05-12T21:52:06.935207+00:00 :
    dt_from_unix = try Datetime.fromUnix(831937926935207836, Duration.Resolution.nanosecond, null, null);
  ...
  // 2039-09-16T17:57:48.298809+00:00 :
    dt_from_unix = try Datetime.fromUnix(2199808668298809902, Duration.Resolution.nanosecond, null, null);
  ...
  // 1985-12-28T03:09:57.313186+00:00 :
    dt_from_unix = try Datetime.fromUnix(504587397313186504, Duration.Resolution.nanosecond, null, null);
  ...
  // 2081-10-21T11:19:41.534193+00:00 :
    dt_from_unix = try Datetime.fromUnix(3528271181534193650, Duration.Resolution.nanosecond, null, null);
  ...
  // 1976-06-24T08:50:13.067365+00:00 :
    dt_from_unix = try Datetime.fromUnix(204454213067365555, Duration.Resolution.nanosecond, null, null);
  ...
  // 2006-04-17T00:29:16.830281+00:00 :
    dt_from_unix = try Datetime.fromUnix(1145233756830281912, Duration.Resolution.nanosecond, null, null);
  ...
  // 1933-11-18T09:46:52.175769+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1139839987824230190, Duration.Resolution.nanosecond, null, null);
  ...
  // 1912-10-13T12:31:05.308802+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1805628534691197833, Duration.Resolution.nanosecond, null, null);
  ...
  // 2081-10-11T19:25:53.550118+00:00 :
    dt_from_unix = try Datetime.fromUnix(3527436353550118265, Duration.Resolution.nanosecond, null, null);
  ...
  // 2060-09-11T07:26:53.914799+00:00 :
    dt_from_unix = try Datetime.fromUnix(2862113213914799238, Duration.Resolution.nanosecond, null, null);
  ...
  // 1996-11-18T20:02:47.772468+00:00 :
    dt_from_unix = try Datetime.fromUnix(848347367772468167, Duration.Resolution.nanosecond, null, null);
  ...
  // 1905-10-09T15:32:55.099978+00:00 :
    dt_from_unix = try Datetime.fromUnix(-2026888024900021709, Duration.Resolution.nanosecond, null, null);
  ...
  // 2077-04-21T06:21:57.695324+00:00 :
    dt_from_unix = try Datetime.fromUnix(3386211717695324281, Duration.Resolution.nanosecond, null, null);
  ...
  // 2047-09-08T08:51:54.574312+00:00 :
    dt_from_unix = try Datetime.fromUnix(2451545514574312538, Duration.Resolution.nanosecond, null, null);
  ...
  // 1915-08-02T22:31:41.958495+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1717205298041504919, Duration.Resolution.nanosecond, null, null);
  ...
  // 2058-10-18T05:59:11.003833+00:00 :
    dt_from_unix = try Datetime.fromUnix(2802146351003833012, Duration.Resolution.nanosecond, null, null);
  ...
  // 1955-09-03T09:52:42.309110+00:00 :
    dt_from_unix = try Datetime.fromUnix(-452182037690889720, Duration.Resolution.nanosecond, null, null);
  ...
  // 1985-10-16T10:01:15.428375+00:00 :
    dt_from_unix = try Datetime.fromUnix(498304875428375353, Duration.Resolution.nanosecond, null, null);
  ...
  // 1989-02-08T18:38:38.127399+00:00 :
    dt_from_unix = try Datetime.fromUnix(602966318127399728, Duration.Resolution.nanosecond, null, null);
  ...
  // 1925-09-11T08:02:20.348743+00:00 :
    dt_from_unix = try Datetime.fromUnix(-1398182259651256224, Duration.Resolution.nanosecond, null, null);
