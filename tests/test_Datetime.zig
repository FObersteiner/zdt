...
test "prev weekday" {
    const dt = try Datetime.fromFields(.{ .year = 1970 }); // a Thu
...
test "datetime from postgres raw data" {
    const pg_epoch = try Datetime.fromUnix(Datetime.us_from_epoch_to_y2k, .microsecond, .{ .tz = &Tz.UTC });
    const want = try Datetime.fromFields(.{ .year = 2000, .tz_options = .{ .tz = &Tz.UTC } });
    var have = try Datetime.fromPgzRow("\x00\x00\x00\x00\x00\x00\x00\x00", 1184);
    try testing.expectEqual(want, pg_epoch);
    try testing.expectEqual(want, have);

    have = try Datetime.fromPgzRow("\x00\x00\x00\x00\x00\x00\x00\x00", 1114);
    try testing.expectEqual(want, have);

    const positive_micros: i64 = 1_000_000;
    var positive_data: [8]u8 = undefined;
    std.mem.writeInt(i64, &positive_data, positive_micros, .big);
    const positive_datetime = try Datetime.fromPgzRow(positive_data[0..], 1184);
    try testing.expectEqual(
        @as(i64, (Datetime.us_from_epoch_to_y2k + positive_micros) / 1_000_000),
        positive_datetime.unix_sec,
    );

    const negative_micros: i64 = -1_000_000;
    var negative_data: [8]u8 = undefined;
    std.mem.writeInt(i64, &negative_data, negative_micros, .big);
    const negative_datetime = try Datetime.fromPgzRow(negative_data[0..], 1184);
    try testing.expectEqual(
        @as(i64, (Datetime.us_from_epoch_to_y2k + negative_micros) / 1_000_000),
        negative_datetime.unix_sec,
    );
}
