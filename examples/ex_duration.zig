...
pub fn main() !void {
    ...
    println("tomorrow, same time : {f}", .{tomorrow});
    ...
    println("two weeks ago : {f}", .{two_weeks_ago});
    ...
    println("in a week and an hour : {f}\n", .{in_a_week});
    ...
    println("{f} --> {f}", .{ dt_dst_off, dt_dst_on });
    println("wall diff: {f}, absolute diff: {f}", .{ try dt_dst_on.diffWall(dt_dst_off), dt_dst_on.diff(dt_dst_off) });
}
