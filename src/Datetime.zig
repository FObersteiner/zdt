...
pub const epoch = Datetime.fromUnix(0, Duration.Resolution.second, Timezone.UTC) catch unreachable;
...
/// UTC offset of the time zone (if such is supplied). Returns corrected datetime.
fn normalize(self: *Datetime) TzError!Datetime {
...
// TODO : next/previous weekday
// /// Roll datetime forward to the specified next weekday.
// pub fn next(self: Datetime, d: Weekday) Datetime {
//     var daysdiff: i4 = 0;
//     if (self.weekday() == d) {
//         daysdiff = 7;
//     } else {
//         daysdiff = util.weekdayDifference(@intFromEnum(d), self.weekday_n());
//         if (daysdiff < 0) daysdiff += 7; // ensure a positive shift since we want 'next'
//     }
//     const offset = Duration.from_sec(@as(i39, s_per_day) * daysdiff);
...
// pub fn previous(self: Datetime, d: Weekday) Datetime {
//     var daysdiff: i4 = 0;
//     if (self.weekday() == d) {
//         daysdiff = -7;
//     } else {
...
pub fn weekday(self: Datetime) Weekday {
    return std.meta.intToEnum(Weekday, self.weekdayNumber()) catch Weekday.Sunday;
}
...
pub fn monthEnum(self: Datetime) Month {
    return std.meta.intToEnum(Month, self.month) catch Month.January;
}

// /// TODO : nth weekday of the month, returned as a Datetime.
// /// nth must be in range [1..5];
// /// although 5 might return an error for certain year/month combinations.
// pub fn nthWeekday(year: u14, month: u4, weekday: zdt.Weekday, nth: u3) zdt.RangeError!zdt.Datetime {
//     if (nth > 5 or nth == 0) return zdt.RangeError.DaysOutOfRange;
//     var dt = try zdt.Datetime.from_fields(year, month, 1, 0, 0, 0, 0, null);
//     if (dt.weekday() != weekday) dt = dt.next(weekday);
//     if (nth == 1) return dt;
//     dt = try dt.add(zdt.Duration{ .__internal = @as(i70, std.time.ns_per_s) * @as(u20, std.time.s_per_day) * 7 * (nth - 1) });
//     if (dt.month != month) return zdt.RangeError.DaysOutOfRange else return dt;
// }

// /// TODO : Week number of the year (Sunday as the first day of the week) as returned from
// /// strftime's %U
// pub fn weekOfYearSun(dt: zdt.Datetime) u7 {
//     const doy = dayOfYear(dt.year, dt.month, dt.day);
//     const dow = dt.weekday_n();
//     return @truncate(@divFloor(doy + 7 - dow, 7));
// }
//
// /// TODO : Week number of the year (Monday as the first day of the week) as returned from
// /// strftime's %W
// pub fn weekOfYearMon(dt: zdt.Datetime) u7 {
//     const doy = dayOfYear(dt.year, dt.month, dt.day);
//     const dow = dt.weekday_n();
...
fn getSurroundingTimetypes(idx: i32, _tz: *const tzif.Tz) [3]?*tzif.Timetype {
