...
test "validate datetime fields" {
    ...
    try testing.expectError(datetime.ZdtError.DayOutOfRange, err);
    ...
    err = fields.validate();
    try testing.expectError(datetime.ZdtError.MonthOutOfRange, err);

    fields = datetime.DatetimeFields{ .year = 10000, .month = 1, .day = 1 };
    err = fields.validate();
    try testing.expectError(datetime.ZdtError.YearOutOfRange, err);

    fields = datetime.DatetimeFields{ .hour = 99 };
    err = fields.validate();
    try testing.expectError(datetime.ZdtError.HourOutOfRange, err);

    fields = datetime.DatetimeFields{ .minute = 99 };
    err = fields.validate();
    try testing.expectError(datetime.ZdtError.MinuteOutOfRange, err);

    fields = datetime.DatetimeFields{ .second = 99 };
    err = fields.validate();
    try testing.expectError(datetime.ZdtError.SecondOutOfRange, err);
}
...
test "validate tz field" {
    ...
    try testing.expectError(datetime.ZdtError.AllTZRulesUndefined, err);
}

test "Datetime from empty field struct" {
    const dt = try datetime.Datetime.fromFields(.{});
    try testing.expect(dt.year == @as(u14, 1));
    try testing.expect(dt.month == @as(u4, 1));
    try testing.expect(dt.day == @as(u5, 1));
...
test "Datetime from populated field struct" {
    ...
    try testing.expect(dt.year == @as(u14, 2023));
    try testing.expect(dt.month == @as(u4, 12));
    try testing.expect(dt.day == @as(u5, 1));
    try testing.expect(dt.tzinfo == null);
}
...
test "Datetime Unix epoch roundtrip" {
    var unix_from_fields = try datetime.Datetime.fromFields(.{ .year = 1970, .second = 42 });
    var unix_from_int = try datetime.Datetime.fromUnix(42, dur.Resolution.second, null);
    try testing.expect(std.meta.eql(unix_from_fields, unix_from_int));
    var unix_s = unix_from_int.toUnix(dur.Resolution.second);
    try testing.expect(unix_s == 42);

    unix_from_fields = try datetime.Datetime.fromFields(.{ .year = 1970, .second = 42, .nanosecond = 1_000_000 });
    unix_from_int = try datetime.Datetime.fromUnix(42001, dur.Resolution.millisecond, null);
    try testing.expect(std.meta.eql(unix_from_fields, unix_from_int));
    unix_s = unix_from_int.toUnix(dur.Resolution.millisecond);
    try testing.expect(unix_s == 42001);

    unix_from_fields = try datetime.Datetime.fromFields(.{ .year = 1970, .second = 42, .nanosecond = 1_001_000 });
    unix_from_int = try datetime.Datetime.fromUnix(42001001, dur.Resolution.microsecond, null);
    try testing.expect(std.meta.eql(unix_from_fields, unix_from_int));
    unix_s = unix_from_int.toUnix(dur.Resolution.microsecond);
    try testing.expect(unix_s == 42001001);

    unix_from_fields = try datetime.Datetime.fromFields(.{ .year = 1970, .second = 42, .nanosecond = 1_001_001 });
    unix_from_int = try datetime.Datetime.fromUnix(42001001001, dur.Resolution.nanosecond, null);
    try testing.expect(std.meta.eql(unix_from_fields, unix_from_int));
    unix_s = unix_from_int.toUnix(dur.Resolution.nanosecond);
    try testing.expect(unix_s == 42001001001);
}

test "Fields can represent leap second, Unix cannot" {
    const dt_from_fields = try datetime.Datetime.fromFields(.{ .year = 1970, .second = 60 });
    try testing.expectEqual(@as(u6, 60), dt_from_fields.second);
    const unix_s = dt_from_fields.toUnix(dur.Resolution.second);
    try testing.expectEqual(@as(i72, 59), unix_s);

    const dt_from_int = try datetime.Datetime.fromUnix(60, dur.Resolution.second, null);
...
test "Dateime from invalid fields" {
    ...
    try testing.expectError(datetime.ZdtError.NanosecondOutOfRange, err);
}
...
test "Datetime Min Max fields vs seconds roundtrip" {
    const max_from_seconds = try datetime.Datetime.fromUnix(datetime.unix_s_max, dur.Resolution.second, null);
    ...
    const min_from_seconds = try datetime.Datetime.fromUnix(datetime.unix_s_min, dur.Resolution.second, null);
    try testing.expect(std.meta.eql(min_from_fields, min_from_seconds));

    const too_large_s = datetime.Datetime.fromUnix(datetime.unix_s_max + 1, dur.Resolution.second, null);
    try testing.expectError(datetime.ZdtError.UnixOutOfRange, too_large_s);
    const too_large_ns = datetime.Datetime.fromUnix(@as(i72, datetime.unix_s_max + 1) * std.time.ns_per_s, dur.Resolution.second, null);
    try testing.expectError(datetime.ZdtError.UnixOutOfRange, too_large_ns);
    const too_small_s = datetime.Datetime.fromUnix(datetime.unix_s_min - 1, dur.Resolution.second, null);
    try testing.expectError(datetime.ZdtError.UnixOutOfRange, too_small_s);
    const too_small_ns = datetime.Datetime.fromUnix(@as(i72, datetime.unix_s_min - 1) * std.time.ns_per_s, dur.Resolution.second, null);
    try testing.expectError(datetime.ZdtError.UnixOutOfRange, too_small_ns);
}
...
test "compare Unix time" {
    ...
    try testing.expectError(datetime.ZdtError.CompareNaiveAware, err);
...
test "compare wall time" {
    ...
    try testing.expectEqual(std.math.Order.eq, try datetime.Datetime.compareWall(a, b));

    a = try datetime.Datetime.fromFields(.{ .year = 2021, .month = 2, .day = 18, .hour = 17, .nanosecond = 42, .tzinfo = null });
    try testing.expectEqual(std.math.Order.lt, try datetime.Datetime.compareWall(a, b));
    try testing.expectEqual(std.math.Order.gt, try datetime.Datetime.compareWall(b, a));
}
...
test "floor naive datetime to the second" {
    ...
    const dt_floored = try dt.floorTo(dur.Timespan.second);
...
test "floor naive datetime to the minute" {
    const dt = try datetime.Datetime.fromFields(.{ .year = 2021, .month = 2, .day = 18, .hour = 18, .minute = 5, .second = 32, .nanosecond = 42, .tzinfo = null });
    const dt_floored = try dt.floorTo(dur.Timespan.minute);
    try testing.expectEqual(dt_floored.year, dt.year);
    try testing.expectEqual(dt_floored.month, dt.month);
    try testing.expectEqual(dt_floored.day, dt.day);
    try testing.expectEqual(dt_floored.hour, dt.hour);
...
test "floor naive datetime to the hour" {
    const dt = try datetime.Datetime.fromFields(.{ .year = 2021, .month = 2, .day = 18, .hour = 18, .minute = 5, .second = 32, .nanosecond = 42, .tzinfo = null });
    const dt_floored = try dt.floorTo(dur.Timespan.hour);
    try testing.expectEqual(dt_floored.year, dt.year);
    try testing.expectEqual(dt_floored.month, dt.month);
    try testing.expectEqual(dt_floored.day, dt.day);
    try testing.expectEqual(dt_floored.hour, dt.hour);
...
test "floor naive datetime to the date" {
    ...
    const dt_floored = try dt.floorTo(dur.Timespan.day);
    try testing.expectEqual(dt_floored.year, dt.year);
    try testing.expectEqual(dt_floored.month, dt.month);
    try testing.expectEqual(dt_floored.day, dt.day);
...
test "day of year" {
    // leap year
    var dt = try datetime.Datetime.fromFields(.{ .year = 2020 });
    var i: u9 = 1;
    while (i < 367) : (i += 1) {
        try testing.expectEqual(i, dt.dayOfYear());
        dt = try dt.add(dur.fromTimespan(1, dur.Timespan.day));
    }
    // normal year
    dt = try datetime.Datetime.fromFields(.{ .year = 2021 });
    i = 1;
    while (i < 366) : (i += 1) {
        try testing.expectEqual(i, dt.dayOfYear());
        dt = try dt.add(dur.fromTimespan(1, dur.Timespan.day));
...
test "day of week" {
    var dt = try datetime.Datetime.fromFields(.{ .year = 1970, .day = 4 });
    var i: u3 = 0;
    while (i < 7) : (i += 1) {
...
test "day of week, iso" {
    var dt = try datetime.Datetime.fromFields(.{ .year = 1970, .day = 5 });
    var i: u4 = 1;
    while (i < 8) : (i += 1) {
...
test "unix nanoseconds, fields" {
    ...
    var dt_from_unix = try datetime.Datetime.fromUnix(-1206205679347298795, dur.Resolution.nanosecond, null);
    ...
    var unix: i72 = dt_from_fields.toUnix(dur.Resolution.nanosecond);
    try testing.expect(unix == -1206205679347298795);

    // 1969-11-24T23:10:10.285143+00:00 :
    dt_from_unix = try datetime.Datetime.fromUnix(-3199789714856270, dur.Resolution.nanosecond, null);
    ...
    try testing.expect(std.meta.eql(dt_from_unix, dt_from_fields));
    unix = dt_from_fields.toUnix(dur.Resolution.nanosecond);
    try testing.expect(unix == -3199789714856270);

    // 1939-10-30T19:50:28.445918+00:00 :
    dt_from_unix = try datetime.Datetime.fromUnix(-952142971554081681, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1285795301506902592, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2703237847794000687, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3109692914335618665, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1922714532679270618, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1365057663128977552, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-113365943056244079, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3213512660899537529, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2846256540839859462, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1569708587657462123, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1837124742687712253, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(296732011101612230, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-770905734280258935, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1597583583388538346, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(293853313501302021, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-269617124548303398, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1288361114439814433, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1213091215937809235, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1024409024710551602, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(889090714682387225, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(173603950766595007, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1817619323464320928, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1929142309426128724, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1084654253138298354, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1499222353706732940, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(431726968301809234, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3453297781849394069, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1048330983191233927, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-477006398441024968, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1582491779321359120, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3747138898920857338, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1214882252757712072, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1874967946635065526, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-743930938719166757, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(991003943603720285, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3827412540694702685, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3947863726618697497, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1565853318838868781, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3510385541079667552, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2602114040929793135, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-3927983821355260, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1954677752602351957, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(222484010327543903, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3989424851019304584, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-230870500269510410, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(711892549386543484, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1705142853611974752, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1404381425887228803, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1612816124605793516, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2899715639601124826, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-293818581509416584, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2287685860209685194, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3581309647792741096, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-922115843116886901, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-951309568929825326, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2645688160170619441, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1649998876560761362, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3047011349370997433, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1051584582810775420, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-963005913104399522, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2236205755510952884, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-832318166706378254, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-767998807527995945, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3920242170992268689, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3162986308308200669, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1256685008501264661, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3158296029900066100, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2006830822010777062, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(55547864246697412, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2774027837634489745, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3918456406920080347, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1177197121070662181, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(855039326909075882, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(434624179343081111, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-784432179960165746, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-2179759598822236804, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2363799222845748194, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1250524273485200451, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3423216007491094459, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2363633696084391143, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-417357756835640568, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1158927641467261187, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-753884964894622715, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3185820838714065473, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2191979681104955255, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1201376503804541105, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(1060451670345777945, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-52196637106432923, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-39411591442661547, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2901308793724088827, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1499606459439241118, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2589201605997253876, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-1076239426053707437, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3733515617280861100, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(-721656443875919949, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(2543874687770691667, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(3254724448691610553, dur.Resolution.nanosecond, null);
    ...
    dt_from_unix = try datetime.Datetime.fromUnix(4004788335289130856, dur.Resolution.nanosecond, null);
