...
test "comptime parse with comptime format string" {
    const cases = [_]TestCase{
        .{
            ...
            .dt = try datetime.Datetime.naiveFromList(.{ 2021, 2, 18, 17, 0, 1, 0 }),
        },
        .{
            .string = "1970-01-01 00:00:00",
            .dt = try datetime.Datetime.naiveFromList(.{ 1970, 1, 1, 0, 0, 0, 0 }),
...
test "comptime parse with fractional part" {
    const cases = [_]TestCase{
        .{
            ...
            .dt = try datetime.Datetime.naiveFromList(.{ 2021, 2, 18, 17, 0, 0, 100_000_000 }),
        },
        .{
            .string = "2021-02-18T17:00:00.12",
            .dt = try datetime.Datetime.naiveFromList(.{ 2021, 2, 18, 17, 0, 0, 120_000_000 }),
        },
        .{
            .string = "2021-02-18T17:00:00.123",
            .dt = try datetime.Datetime.naiveFromList(.{ 2021, 2, 18, 17, 0, 0, 123_000_000 }),
        },
        .{
            .string = "2021-02-18T17:00:00.1234",
            .dt = try datetime.Datetime.naiveFromList(.{ 2021, 2, 18, 17, 0, 0, 123_400_000 }),
        },
        .{
            .string = "2021-02-18T17:00:00.12345",
            .dt = try datetime.Datetime.naiveFromList(.{ 2021, 2, 18, 17, 0, 0, 123_450_000 }),
        },
        .{
            .string = "2021-02-18T17:00:00.123456",
            .dt = try datetime.Datetime.naiveFromList(.{ 2021, 2, 18, 17, 0, 0, 123_456_000 }),
        },
        .{
            .string = "2021-02-18T17:00:00.1234567",
            .dt = try datetime.Datetime.naiveFromList(.{ 2021, 2, 18, 17, 0, 0, 123_456_700 }),
        },
        .{
            .string = "2021-02-18T17:00:00.12345678",
            .dt = try datetime.Datetime.naiveFromList(.{ 2021, 2, 18, 17, 0, 0, 123_456_780 }),
        },
        .{
            .string = "2021-02-18T17:00:00.123456789",
            .dt = try datetime.Datetime.naiveFromList(.{ 2021, 2, 18, 17, 0, 0, 123_456_789 }),
        },
    };
...
test "parse single digits" {
    const cases = [_]TestCase{
        .{ // single digit y m d
            ...
            .dt = try datetime.Datetime.naiveFromList(.{ 1, 1, 1, 0, 0, 0, 0 }),
        },
...
test "parse with literal characters" {
    ...
    cases = [_]TestCase{
        .{
            ...
            .dt = try datetime.Datetime.naiveFromList(.{ 2021, 2, 18, 12, 34, 56, 0 }),
        },
        .{
            .string = "1970-01-01 00%00%00",
            .dt = try datetime.Datetime.naiveFromList(.{ 1970, 1, 1, 0, 0, 0, 0 }),
...
test "parse with z" {
    ...
    const off_abbr = @as([]const u8, dt.tzinfo.?.abbreviation[0..1]);
    try testing.expectEqualStrings("Z", off_abbr);
}
...
test "string -> datetime -> string roundtrip with offset TZ" {
    ...
    try testing.expectEqual(@as(u8, 0), dt.tzinfo.?.abbreviation[0]);
}

test "parse ISO" {
    var dt_ref = try datetime.Datetime.fromFields(.{ .year = 2014, .month = 8 });
    var dt = try str.parseISO8601("2014-08");
    try testing.expect(std.meta.eql(dt_ref, dt));

    dt_ref = try datetime.Datetime.fromFields(.{ .year = 2014, .month = 8, .day = 23 });
    dt = try str.parseISO8601("2014-08-23");
    try testing.expect(std.meta.eql(dt_ref, dt));

    dt_ref = try datetime.Datetime.fromFields(.{ .year = 2014, .month = 8, .day = 23, .hour = 12, .minute = 15 });
    dt = try str.parseISO8601("2014-08-23 12:15");
    try testing.expect(std.meta.eql(dt_ref, dt));

    dt_ref = try datetime.Datetime.fromFields(.{ .year = 2014, .month = 8, .day = 23, .hour = 12, .minute = 15, .second = 56 });
    dt = try str.parseISO8601("2014-08-23 12:15:56");
    try testing.expect(std.meta.eql(dt_ref, dt));

    dt_ref = try datetime.Datetime.fromFields(.{ .year = 2016, .month = 12, .day = 31, .hour = 23, .minute = 59, .second = 60 });
    dt = try str.parseISO8601("2016-12-31T23:59:60");
    try testing.expect(std.meta.eql(dt_ref, dt));

    dt_ref = try datetime.Datetime.fromFields(.{ .year = 2014, .month = 8, .day = 23, .hour = 12, .minute = 15, .second = 56, .nanosecond = 123400000 });
    dt = try str.parseISO8601("2014-08-23 12:15:56,1234");
    try testing.expect(std.meta.eql(dt_ref, dt));

    dt_ref = try datetime.Datetime.fromFields(.{ .year = 2014, .month = 8, .day = 23, .hour = 12, .minute = 15, .second = 56, .nanosecond = 123000000 });
    dt = try str.parseISO8601("2014-08-23 12:15:56,123");
    try testing.expect(std.meta.eql(dt_ref, dt));

    dt_ref = try datetime.Datetime.fromFields(.{ .year = 2014, .month = 8, .day = 23, .hour = 12, .minute = 15, .second = 56, .nanosecond = 123456000 });
    dt = try str.parseISO8601("2014-08-23 12:15:56,123456");
    try testing.expect(std.meta.eql(dt_ref, dt));

    var tzinfo = tz.UTC;
    dt_ref = try datetime.Datetime.fromFields(.{ .year = 2014, .month = 8, .day = 23, .hour = 12, .minute = 15, .second = 56, .tzinfo = tzinfo });
    dt = try str.parseISO8601("2014-08-23 12:15:56Z");
    try testing.expect(std.meta.eql(dt_ref, dt));

    dt_ref = try datetime.Datetime.fromFields(.{ .year = 2014, .month = 8, .day = 23, .hour = 12, .minute = 15, .second = 56, .nanosecond = 123400000, .tzinfo = tzinfo });
    dt = try str.parseISO8601("2014-08-23 12:15:56.1234Z");
    try testing.expect(std.meta.eql(dt_ref, dt));

    dt_ref = try datetime.Datetime.fromFields(.{ .year = 2014, .month = 8, .day = 23, .hour = 12, .minute = 15, .second = 56, .nanosecond = 99, .tzinfo = tzinfo });
