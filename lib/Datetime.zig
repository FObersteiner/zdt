...
pub fn fromUnix(quantity: i128, resolution: Duration.Resolution, tz_ptr: ?*const Timezone) ZdtError!Datetime {
...
pub fn nowUTC() Datetime {
    const t = std.time.nanoTimestamp();
    return Datetime.fromUnix(@intCast(t), Duration.Resolution.nanosecond, null) catch unreachable;
}
...
pub fn tzName(dt: *const Datetime) []const u8 {
    ...
    return dt.utc_offset.originName();
}

pub fn tzAbbreviation(dt: *const Datetime) []const u8 {
    return dt.utc_offset.abbreviation();
}
